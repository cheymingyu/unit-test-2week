...
public class MockServiceTest {
    ...
    @Test
    public void shouldAtLeastTwoTimesForChampionsGetName() {
        Champion topChamp = mock(Champion.class);
        topChamp.setName("티모");
        topChamp.setPosition("탑");
        topChamp.setHasSkinCount(3);
...
//        topChamp.setName("티모");
        ...
        verify(topChamp, never()).setName(any(String.class));
    }

    // 4-4. champion 객체에서 이름을 가져오는 로직이 200ms 시간 이내에 1번 실행되었는 지 검증하는 로직을 작성하세요.
    @Test
    public void shouldOneTimeRunForChampionsGetName() {
        Champion topChamp = mock(Champion.class);
        topChamp.setName("티모");
        topChamp.setPosition("탑");
        topChamp.setHasSkinCount(3);
        ...
        verify(topChamp, timeout(200).times(1)).getName();
    }
    ...
    @Test
    public void 챔피언List사이즈확인() {
        List<Champion> champions = new ArrayList<>();

        champions.add(new Champion("루시안", "원딜", 5));
        champions.add(new Champion("직스", "미드", 4));
        champions.add(new Champion("가렌", "탑", 2));
    ...
    public void 가짜챔피언객체리턴() {
        when(mockService.findByName(anyString())).thenReturn(new Champion("가렌", "탑", 4));
        assertThat(mockService.findByName("애쉬").getHasSkinCount(), is(4));
        verify(mockRepository, times(1)).findByName(anyString());
    }
    ...
    public void 여러개호출회수검증() {
        ...
        assertThat(mockService.findAllChampions().get(2).getName(), is("가렌"));
